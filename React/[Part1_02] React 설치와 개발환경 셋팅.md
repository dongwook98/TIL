...
html 파일에 직접 리액트 설치해도 되는데..

그렇게 하면 시간이 너무 오래걸리고 힘듬 (코딩포기자됨)

<br>

**Create React App** 이라는 라이브러리 도움을 받아서

프로젝트를 생성하는게 일반적이다.

<br>

이 라이브러리를 쓰고싶다면 npm 이라는게 필요한데

npm은 라이브러리를 다 모아놓은 플랫폼 같은거다.

<br>

npm을 쓰고싶으면 nodejs 설치하면 딸려온다.
...
프로젝트 생성해주는 명령어를 제공해준다.

그래서 우리도 쉽게 react 프로젝트를 만들어 본 것이다.
...
**node_modules** - 프로젝트 구동에 필요한 모든 라이브러리 소스코드를 다 모아놓은 폴더 (일반적으로 그냥 놔두면 됨)

**public** - html파일, 이미지파일 그런거 잠깐 보관하고싶을때 쓰는 static 파일 보관함이다. (아직 건들일 없음)

**src** - 여기다가 코드짜면 된다. 우리의 소스코드 보관함이다. 대표적으로 App.js가 우리의 메인페이지이다.

<br>

근데 이상한 점 하나가 있다.

보통 웹페이지는 html파일로 이루어져있는데 지금 js파일에 html 코드 짯는데도 보여준다.

그 이유는 App.js에 있는 html들을 index.html이라는 우리의 메인페이지에다가

"집어 넣어주세요" 라고 누군가가 해주고 있어서 그런것이다.

index.js 라는 파일이 그 역할을 하고 있다.

<br>

**pacakege.json** - 프로젝트 정보
